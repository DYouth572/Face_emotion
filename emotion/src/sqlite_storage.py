...
class SQLiteStorage:
    ...
    def init_schema(self):
        ...
        self._ensure_frame_metrics_au_columns()
    ...
    def _ensure_frame_metrics_au_columns(self):
        cols = [r[1] for r in self.conn.execute("PRAGMA table_info(Frame_metrics)").fetchall()]
        if not cols:
            return
        ...
        for column in AU_COLUMNS:
            if column not in cols:
                self.conn.execute(f"ALTER TABLE Frame_metrics ADD COLUMN {column} REAL")

        if "raw_facs_combination" not in cols:
            self.conn.execute("ALTER TABLE Frame_metrics ADD COLUMN raw_facs_combination TEXT")
            if "state_text" in cols:
                self.conn.execute(
                    "UPDATE Frame_metrics SET raw_facs_combination = state_text WHERE raw_facs_combination IS NULL"
                )

        self.conn.execute(
            """
            UPDATE Frame_metrics
            SET emotion_confidence = emotion_confidence / 100.0
            WHERE emotion_confidence > 1.0
            """
        )
        self.conn.commit()
    ...
    def insert_frame_metrics(
        ...
        state_text: str = None,
        au_scores: dict = None,
        raw_facs_combination: str = None,
    ):
        normalized_confidence = None
        if emotion_confidence is not None:
            normalized_confidence = float(emotion_confidence)
        ...
        scores = au_scores or {}
        au_values = [
            scores.get(column.upper(), scores.get(column))
            for column in AU_COLUMNS
        ]
        raw_facs = raw_facs_combination if raw_facs_combination is not None else state_text
        ...
        self.conn.execute(
            ...
            INSERT INTO Frame_metrics(
                ...
                ear_l, ear_r, mar, brow_ratio, cheek_ratio, head_turn_ratio,
                au1, au2, au4, au5, au6, au7, au9, au10, au12, au15, au20, au23, au25, au26,
                emotion_label, emotion_confidence, raw_facs_combination, state_text
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                ear_l, ear_r, mar, brow_ratio, cheek_ratio, head_turn_ratio,
                *au_values,
                emotion_label, normalized_confidence, raw_facs, state_text
            )
        )
